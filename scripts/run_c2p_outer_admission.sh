#!/usr/bin/env bash
# Run matched 4-SM-local-first C2P and C2P outer-admission replays.
set -euo pipefail
...
run_one() {
  ...
  local args=(--trace "$trace" --config "$base_config"
      ...
      --mode-config-extra "$order_config" --strip-mem-addr-mapping
      --skip-complete --modes c2p --out-dir "$out_root/$case_name/$variant")
  [[ "$variant" == policy ]] && args+=(--mode-config-extra "$policy_config")
...
run_pair() {
  ...
  run_one "$case_name" "$trace" control >"$out_root/$case_name.control.driver.log" 2>&1 &
  local control_pid=$!
  run_one "$case_name" "$trace" policy >"$out_root/$case_name.policy.driver.log" 2>&1 &
  local policy_pid=$!
  wait "$control_pid" || status=1
  wait "$policy_pid" || status=1
  return "$status"
}
