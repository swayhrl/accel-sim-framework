#!/usr/bin/env bash
# Run matched C2P group4 locality ordering/latency sensitivity points.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage: scripts/run_c2p_locality_sensitivity.sh --out-root DIR
       [--trace-root DIR] [--v100-stage DIR] [--case CASE[,CASE...]] [--jobs N]

For each selected workload, replay canonical C2P with: uniform timing,
local-first ordering with no distance surcharge, and outer request/return
surcharges of two and four cycles.  All points keep the same C2P core.
EOF
}
...
declare -A overlays=(
  [uniform]="$repo_root/configs/c2p-cache/c2p-locality-observe.config"
  [near_d0]="$repo_root/configs/c2p-cache/c2p-locality-near-outer-d0.config"
  [near_d2]="$repo_root/configs/c2p-cache/c2p-locality-near-outer-d2.config"
  [near_d4]="$repo_root/configs/c2p-cache/c2p-locality-near-outer-d4.config"
)
for path in "$base_config" "$trace_config" "$paper_config" "$canonical_manifest" \
            "$extension_manifest" "${overlays[@]}"; do
...
run_one() {
  ...
  "$repo_root/scripts/run_c2p_cache_cases.sh" --trace "$trace" \
      --config "$base_config" --config-extra "$trace_config" \
      --config-extra "$paper_config" --mode-config-extra "${overlays[$variant]}" \
      --strip-mem-addr-mapping --skip-complete --modes c2p \
      --out-dir "$out_root/$case_name/$variant"
}

run_quad() {
  local case_name="$1" trace="$2" status=0 variant
  local lock_root="$out_root/.quad_locks" lock_fd
  mkdir -p "$lock_root"
  exec {lock_fd}>"$lock_root/$case_name.lock"
  flock "$lock_fd"
  [[ -f "$trace" ]] || { echo "missing trace: $trace" >&2; return 2; }
  local -a pids=()
  for variant in uniform near_d0 near_d2 near_d4; do
...
python3 "$repo_root/scripts/analyze_c2p_locality_sensitivity.py" \
  --root "$out_root" --case "$case_list" \
  --csv "$out_root/sensitivity.csv" --markdown "$out_root/sensitivity.md"
