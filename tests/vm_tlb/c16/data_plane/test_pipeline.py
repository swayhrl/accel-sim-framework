...
class T(unittest.TestCase):
 ...
 def test_T8_publish_dry(self):a=publish_argv('hrl174new','/dest',run_id('m','s','p','i','t'),'/src');self.assertIn('--append-verify',a);self.assertNotIn('--delete',a);self.assertIn('.partial/',a[-1]);self.assertNotIn('-a',a);self.assertTrue({'--no-owner','--no-group','--no-perms','--omit-dir-times'}.issubset(a))
