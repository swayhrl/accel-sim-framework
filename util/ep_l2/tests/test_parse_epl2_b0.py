...
EPL2L1V1|scope=application|kernel_uid=18446744073709551615|accesses=9|misses=3|bank_conflicts=2
...
with tempfile.TemporaryDirectory() as temp:
    ...
    l1 = list(csv.DictReader((out / "target_l1.csv").open()))
    assert len(l1) == 1 and l1[0]["accesses"] == "9"
    dram = list(csv.DictReader((out / "target_dram.csv").open()))
    assert next(row for row in dram if row["scope"] == "application")["bandwidth_util"] == "0.5"
    window = next(row for row in dram if row["scope"] == "window")
    assert window["interval"] == "5000_cycle"
