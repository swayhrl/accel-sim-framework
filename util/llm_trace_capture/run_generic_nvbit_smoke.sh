...
if [[ $dry_run == 1 ]]; then printf 'DRY-RUN nvcc=%s/bin/nvcc\ninject=%s\npost=%s\noutput=%s\n' "$cuda_home" "$tracer" "$post" "$out"; exit 0;fi
[[ -x "$cuda_home/bin/nvcc" && -f "$tracer" && -x "$post" ]] || { echo 'missing nvcc/tracer/postprocessor' >&2;exit 1; }
mkdir -p "$out/traces"; "$cuda_home/bin/nvcc" "$framework_root/util/llm_trace_capture/nvbit_generic_smoke.cu" -o "$out/smoke"
