...
Usage (only after explicit M4A-C authorization):
  M4A_C_AUTHORIZED=1 run_m4a_c.sh --framework-root DIR --work-root DIR \
    --workload-command-file FILE [--minimum-free-gib N]
...
framework_root=""; work_root=""; command_file=""; minimum_free_gib=500
while [[ $# -gt 0 ]]; do
  case "$1" in
...
python3 "$script_dir/preflight.py" --framework-root "$framework_root" --work-root "$work_root" --minimum-free-gib "$minimum_free_gib"
...
mkdir -p "$trace_dir"
export M4A_RUN_DIR="$run_dir" M4A_METADATA_PATH="$run_dir/allocation-sidecar.json" CUDA_VISIBLE_DEVICES=0
...
export M4A_PHASE=trace TRACES_FOLDER="$run_dir" CUDA_INJECTION64_PATH="$tracer" TOOL_COMPRESS=1 TRACE_FILE_COMPRESS=1
