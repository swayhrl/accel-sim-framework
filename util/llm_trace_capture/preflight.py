...
def main() -> int:
    ...
    parser.add_argument("--required-gpu-count", type=int, default=1)
    args = parser.parse_args()
    root, work = args.framework_root.resolve(), args.work_root.resolve()
    if args.minimum_free_gib < 1 or args.required_gpu_count < 1:
        parser.error("GPU count and minimum free GiB must be positive")
    ...
    if gpu_rc == 0:
        if len(rows) != args.required_gpu_count:
            errors.append(f"expected {args.required_gpu_count} visible GPU(s), saw {len(rows)}")
        elif any("8.6" not in row for row in rows):
            errors.append(f"paper route requires every GPU to be SM86; got {rows}")
