...
STOP = False
ANALYSIS_CONTRACT_SCHEMA = 'm4i-range-index-sorted-v1'
...
def digest(p):
 ...
 return h.hexdigest()
def analysis_contract(input_root,roi):
 sidecar=input_root/'allocation-sidecar.json'
 analyzer=Path(__file__).with_name('analyze_trace_address_coverage.py')
 # new_output reconstructs the exact ROI-filtered, globally sorted range map.
 _,ranges_data=new_output(input_root,roi)
 range_map=[{'start':hex(a),'end_exclusive':hex(b),'kind':k} for a,b,k in ranges_data]
 contract={'schema':ANALYSIS_CONTRACT_SCHEMA,'roi':roi,
           'analyzer_sha256':digest(analyzer),'sidecar_sha256':digest(sidecar),
           'range_map':range_map}
...
def atomic(path,obj):
 ...
 os.replace(tmp,path)
def encode(o,trace,sha,contract):
 ...
 o['address_format_counts']=dict(o['address_format_counts']);o['trace']={'path':str(trace),'filename':trace.name,'sha256':sha};o['analysis_contract']=contract;return o
...
def aggregate(input_root,work,roi,files,contract):
 o,rs=new_output(input_root,roi)
 for f in files:
  p=work/'partial'/(f.name+'.json');x=json.loads(p.read_text());
  if x['trace']['sha256']!=digest(f) or x.get('analysis_contract')!=contract:raise RuntimeError('partial identity mismatch '+str(f))
 ...
 out['partial_set_sha256']=hashlib.sha256(''.join(digest(work/'partial'/(f.name+'.json')) for f in files).encode()).hexdigest();out['analysis_contract']=contract;return out
def partial_output(work,f,sha,contract):
 p=work/'partial'/(f.name+'.json')
 if not p.exists(): return None
 try: x=json.loads(p.read_text())
 except (OSError,json.JSONDecodeError): return None
 if x.get('trace',{}).get('path')!=str(f) or x.get('trace',{}).get('filename')!=f.name or x.get('trace',{}).get('sha256')!=sha or x.get('analysis_contract')!=contract:return None
...
def build_partial(trace_s, ranges_data, roi, part_s, contract):
 """Independent worker: one immutable input produces one atomic partial."""
 f=Path(trace_s); sha=digest(f)
 atomic(Path(part_s),encode(scan_file(f,ranges_data,roi),f,sha,contract))
 return str(f),sha
def main():
 ...
 files=sorted((a.input/'traces').glob('*.traceg.xz'));work=a.work/a.roi;start=time.time();pid=os.getpid();contract=analysis_contract(a.input,a.roi)
 def halt(sig,_):
 ...
 atomic(work/'run.log',{'command':sys.argv,'pid':pid,'start_timestamp':now(),'input':str(a.input),'roi':a.roi,'analysis_contract':contract})
 done=0
 try:
  o,rs=new_output(a.input,a.roi); pending=[]
  # Validate every pre-existing partial against its immutable input before skip.
  for f in files:
   sha=digest(f);x=partial_output(work,f,sha,contract)
  ...
  if pending and a.workers==1:
   for f in pending:
    ...
    _,sha=build_partial(str(f),rs,a.roi,str(work/'partial'/(f.name+'.json')),contract);x=partial_output(work,f,sha,contract)
    if x is None:raise RuntimeError('atomic partial validation failed '+str(f))
    merge_output(o,x);done+=1;report(f.name)
  elif pending:
   with ProcessPoolExecutor(max_workers=a.workers) as pool:
    jobs={pool.submit(build_partial,str(f),rs,a.roi,str(work/'partial'/(f.name+'.json')),contract):f for f in pending}
    for job in as_completed(jobs):
     if STOP:
      for other in jobs:other.cancel()
      raise KeyboardInterrupt
     f=jobs[job];_,sha=job.result();x=partial_output(work,f,sha,contract)
     if x is None:raise RuntimeError('atomic partial validation failed '+str(f))
     merge_output(o,x);done+=1
     if done==len(files) or done%16==0:report(f.name)
  else: report('all pre-existing partials validated')
  one=aggregate(a.input,work,a.roi,files,contract);two=aggregate(a.input,work,a.roi,files,contract)
