#!/usr/bin/env bash
set -euo pipefail
run=/data/c16/awma/simcompat-v2/route_b/r2_tiny_frozen_live_20260917T153355Z
...
if xz -dc "$trace" | grep -F ' 2 0x' >/dev/null; then exit 1; fi
...
sha256sum "$run/R2_RECEIPT.txt" "$run/frozen_parser.stdout" "$run/frozen_parser.stderr" >> "$run/SHA256SUMS"
