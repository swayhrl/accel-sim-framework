...
Usage: run_m4c_replay.sh --framework-root DIR --core-root DIR --simulator PATH \
  --roi {prefill|decode1} --profile {disabled|ideal|generic|paper|subentry|subentry-segment} \
...
[[ "$profile" == disabled || "$profile" == ideal || "$profile" == generic || "$profile" == paper || "$profile" == subentry || "$profile" == subentry-segment ]] || { echo "invalid profile" >&2; exit 2; }
...
case "$roi" in
  prefill)
    object_map="$framework_root/configs/vm_tlb/object_maps/M4C_PREFILL_OBJECT_MAP.tsv"
    segment_map="$framework_root/configs/vm_tlb/segment_maps/M4B_PREFILL_WEIGHT_SEGMENT_MAP.tsv"
    ;;
  decode1)
    object_map="$framework_root/configs/vm_tlb/object_maps/M4C_DECODE1_OBJECT_MAP.tsv"
    segment_map="$framework_root/configs/vm_tlb/segment_maps/M4B_DECODE1_WEIGHT_SEGMENT_MAP.tsv"
    ;;
esac
case "$profile" in
  ...
  subentry-segment) profile_config="$framework_root/configs/vm_tlb/M4B_SPECULATIVE_SUBENTRY16_WEIGHT_SEGMENT.config" ;;
esac
...
if [[ "$profile" == subentry-segment ]]; then
  [[ -f "$segment_map" ]] || { echo "missing immutable Segment map: $segment_map" >&2; exit 2; }
fi
...
if [[ "$profile" == subentry-segment ]]; then
  printf '%s\n' "-gpgpu_vm_weight_segment_map $segment_map" >> "$run_dir/gpgpusim.config"
fi

{
  ...
  if [[ "$profile" == subentry-segment ]]; then
    sha256sum "$segment_map" | awk '{print "sha256:" $2 "\t" $1}'
  fi
