...
PAYLOAD=Path('/data/c16/deepseek_v27/authority/deepseek-v2-lite__S3_TEXT.json')
OUT=Path('/data/c16/deepseek_v27/state');OUT.mkdir(parents=True,exist_ok=True)
def sha_file(p):
 h=hashlib.sha256()
...
if sha_file(PAYLOAD)!='66fa598b03432cbd67f7fc7d469abe647f33ece260e59d0892d7714836c2a96e' or len(ids)!=8192:raise RuntimeError('input authority mismatch')
...
with torch.inference_mode():
 ...
 pos=torch.tensor([[len(ids)]],device='cuda');mask=torch.zeros((1,1,1,len(ids)+1),device='cuda',dtype=h.dtype)
...
def observed_matmul(a,b,*args,**kwargs):
 ...
 if isq and os.environ.get('C16_V27_NVTX')=='1':torch.cuda.nvtx.range_push('C16_V27_MIXED_PERSISTENT_QK_INCONTEXT')
 out=old_matmul(a,b,*args,**kwargs)
 if isq and os.environ.get('C16_V27_NVTX')=='1':torch.cuda.synchronize();torch.cuda.nvtx.range_pop()
...
try:
 with torch.inference_mode():l2(h2,attention_mask=torch.zeros((1,1,1,len(ids)+2),device='cuda',dtype=h2.dtype),position_ids=torch.tensor([[len(ids)+1]],device='cuda'),past_key_value=cache,output_attentions=False,use_cache=True)[0];torch.cuda.synchronize()
finally:off(l2)
