...
H=struct.Struct('<8sIIQQQ');R=struct.Struct('<6I32Q');base=Path('/data/c16/deepseek_v27');root=base/'QK_formal_capture';recovery=base/'QK_overflow_recovery';recovered={1060,1063,1065,1066};static={int(x['nvbit_static_index']):x for x in csv.DictReader((base/'QK_static.tsv').open(),delimiter='\t')};term=re.compile(r'C16_WARP_TERMINAL static=(\d+) occurrence=(\d+) records=(\d+) overflow=(\d+)')
def sha(p):
 h=hashlib.sha256();
...
dirs=[]
for d in root.glob('mref_*'):
 i=int(d.name[5:]);dirs.append((recovery/f'mref_{i}') if i in recovered else d)
for d in sorted(dirs,key=lambda x:int(x.name[5:])):
...
r={'target':'layer0.self_attn.QK_matmul','evidence_class':'MIXED_PERSISTENT_CACHE_CONSUMER','static_direct_global_mref_count':len(rows),'executed_shards':sum(x['classification']=='EXECUTED_SHARD' for x in rows),'zero_shards':sum(x['classification']!='EXECUTED_SHARD' for x in rows),'overflow_total':sum(x['overflow'] for x in rows),'active_lane_events':events,'union_cta_count':len(ctas),'union_cta_x_min':min(x[0] for x in ctas),'union_cta_x_max':max(x[0] for x in ctas),'union_cta_z_values':sorted({x[2] for x in ctas}),'full_scope_gate':'PASS' if len(rows)==131 and not sum(x['overflow'] for x in rows) and ctas else 'FAIL','shards':rows}
