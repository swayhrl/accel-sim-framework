...
static std::string target,out; static unsigned long long want=0,ord=0,cap=0; static C16Rec* rec=nullptr; static C16Ctr* ctr=nullptr; static bool installed=false;
...
void nvbit_at_cuda_event(CUcontext c,int exit,nvbit_api_cuda_t k,const char*,void*p,CUresult*){if(exit)return;CUfunction f;if(!launch(k,p,&f))return;ord++;if(target!=std::string(nvbit_get_func_name(c,f,true)))return;setup(c,f);nvbit_enable_instrumented(c,f,ord==want);}
