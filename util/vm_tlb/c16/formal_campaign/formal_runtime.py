...
def main():
 a=argparse.ArgumentParser();a.add_argument('--deployment',required=True,choices=['qwen05','qwen7awq','llama']);a.add_argument('--token-ids',type=Path,required=True);a.add_argument('--object-map-out',type=Path,required=True);a.add_argument('--receipt-out',type=Path,required=True);a.add_argument('--decode-steps',type=int,required=True);x=a.parse_args()
