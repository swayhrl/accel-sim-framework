...
static WState* st=nullptr; static std::string fn,out; static unsigned pick=0,selected=0; static unsigned long long cap=0; static int cb=0,ce=2147483647; static bool installed=false,skip=false,done=false;
...
static void install(CUcontext c,CUfunction f){if(installed)return;for(auto*i:nvbit_get_instrs(c,f)){if(i->getIdx()!=pick||i->getMemorySpace()!=InstrType::MemorySpace::GLOBAL||!mref(i))continue;for(int k=0;k<i->getNumOperands();k++)if(i->getOperand(k)->type==InstrType::OperandType::MREF){nvbit_insert_call(i,"c16_warp_record",IPOINT_BEFORE);nvbit_add_call_arg_guard_pred_val(i);nvbit_add_call_arg_mref_addr64(i,k);nvbit_add_call_arg_const_val32(i,pick);nvbit_add_call_arg_const_val64(i,(uint64_t)st);nvbit_add_call_arg_const_val64(i,cap);nvbit_add_call_arg_const_val32(i,cb);nvbit_add_call_arg_const_val32(i,ce);} }installed=true;}
void nvbit_at_init(){const char*a=getenv("C16_WARP_FUNCTION"),*b=getenv("C16_WARP_OUTPUT"),*c=getenv("C16_WARP_STATIC"),*d=getenv("C16_WARP_FUNCTION_OCCURRENCE"),*e=getenv("C16_WARP_CAPACITY");if(!a||!b||!c||!d||!e)abort();fn=a;out=b;pick=strtoul(c,0,10);selected=strtoul(d,0,10);cap=strtoull(e,0,10);if(getenv("C16_CTA_BEGIN"))cb=atoi(getenv("C16_CTA_BEGIN"));if(getenv("C16_CTA_END"))ce=atoi(getenv("C16_CTA_END"));}
