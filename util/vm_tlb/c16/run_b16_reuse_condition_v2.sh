...
start_epoch=$(date +%s)
core_head_at_launch=$(git -C "$core" rev-parse HEAD)
framework_head_at_launch=$(git -C "$framework" rev-parse HEAD)
...
printf '\n' >> COMMAND.txt
cat > LAUNCH_AUTHORITY.json <<EOF
{
  "schema": "C16_E1_B16_REUSE_LAUNCH_AUTHORITY_V1",
  "condition": "$condition",
  "start_utc": "$start_utc",
  "core_head_at_launch": "$core_head_at_launch",
  "framework_head_at_launch": "$framework_head_at_launch",
  "binary_sha256": "$binary_sha256",
  "config_sha256": "$config_sha256",
  "trace_config_sha256": "$trace_config_sha256",
  "kernelslist_sha256": "$kernelslist_sha256",
...
{
  "schema": "C16_E1_B16_REUSE_RUN_RECEIPT_V2",
  "condition": "$condition",
  "status": "$status",
  "exit_code": $exit_code,
  ...
  "kernelslist_sha256": "$kernelslist_sha256",
  "runner_sha256": "$runner_sha256",
  "launch_authority_sha256": "$(sha256sum LAUNCH_AUTHORITY.json | awk '{print $1}')",
  "run_class": "$( [[ $condition == M1_B16_DIAGNOSTIC ]] && echo DIAGNOSTIC || echo PRIMARY )"
}
EOF
sha256sum simulator.stdout simulator.stderr TIME.txt COMMAND.txt LAUNCH_AUTHORITY.json RUN_RECEIPT.json > OUTPUT_SHA256SUMS
exit $exit_code
