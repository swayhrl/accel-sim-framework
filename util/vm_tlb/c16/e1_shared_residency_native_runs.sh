...
OUT=/data/c16/e1_shared_residency_feasibility_v1/shared/native
PYTHON=/data/c16/env/c16-awq-v6/bin/python

exec 9>/data/c16/locks/c16_gpu_campaign.lock
if ! flock -n 9; then
  echo "GPU lock is held; refusing shared-residency native runs" >&2
  exit 75
fi
...
for condition in SETASIDE_ONLY ROTATE_CONTROL_3 SINGLE_L0_UP SHARE2_UP SHARE2_L0 SHARE3; do
  ...
  echo "PASS shared native $condition"
done
