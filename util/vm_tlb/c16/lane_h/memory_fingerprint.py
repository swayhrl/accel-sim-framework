...
line buckets, and cache-set projections are structural properties of that
...
def load_manifest(path: Path, expected_sha256: str) -> tuple[str, list[ManifestEntry]]:
    ...
    for raw in entries_raw:
        ...
        if raw.get("address_domain") != ADDRESS_DOMAIN:
            raise TraceManifestError("only GPU_VA_OBSERVED capture entries are valid for this fingerprint")
...
class MetricAccumulator:
    ...
    cache_sets: set[int] = field(default_factory=set)
    ...
    def add(self, event: MemoryEvent, lane: LaneAccess, cache_set_count: int | None, allow_local_order: bool) -> None:
        ...
        pages_4k = list(_units(lane.address, lane.width, 4 * 1024))
        ...
        if cache_set_count is not None:
            self.cache_sets.update(line % cache_set_count for line in lines)
...
class WindowResult:
    ...
    object_lines: dict[str, set[int]]
    object_pages_4k: dict[str, set[int]]
    object_pages_64k: dict[str, set[int]]
...
def _process_event(
    ...
    object_pages_4k: dict[str, set[int]],
    object_pages_64k: dict[str, set[int]],
    cache_set_count: int | None,
    allow_local_order: bool,
) -> int:
    if not event.lanes:
        return 1
    for lane in event.lanes:
        classification = object_map.classify(lane.address, lane.width)
        object_class = classification.object_class
        key = (object_class, event.access_kind)
        accumulator = metrics.setdefault(key, MetricAccumulator())
        accumulator.add(event, lane, cache_set_count, allow_local_order)
        object_lines.setdefault(object_class, set()).update(_units(lane.address, lane.width, 128))
        object_pages_4k.setdefault(object_class, set()).update(_units(lane.address, lane.width, 4 * 1024))
        object_pages_64k.setdefault(object_class, set()).update(_units(lane.address, lane.width, 64 * 1024))
    return 0


def fingerprint_entry(entry: ManifestEntry, manifest_sha256: str, object_map: RuntimeObjectMapV2, cache_set_count: int | None) -> WindowResult:
    ...
    def process(record: tuple[int, str]) -> None:
        ...
        if event is not None:
            zero_active_memory_events += _process_event(
                event, object_map, metrics, object_lines, object_pages_4k, object_pages_64k,
                cache_set_count, entry.order_model == "LOCAL_STREAM_ORDER",
            )
...
FINGERPRINT_COLUMNS = [
    "manifest_sha256", "trace_sha256", "run_id", "deployment_id", "scenario_id", "phase", "kernel_identity",
    "window_ordinal", "address_domain", "evidence_tier", "capture_status", "terminal_status", "parse_status",
    "order_model", "object_class", "access_kind", "memory_event_count", "lane_references", "requested_bytes",
    "unique_byte_footprint", "unique_4k_va_buckets", "unique_64k_va_buckets", "unique_128b_lines",
    "unique_32b_sectors", "sector_utilization_proxy", "cache_set_count", "unique_cache_sets",
...
def fingerprint_rows(result: WindowResult, cache_set_count: int | None) -> Iterator[dict[str, str | int | float]]:
    entry = result.entry
    for (object_class, kind), accumulator in sorted(result.metrics.items()):
        ...
        yield {
            ...
            "unique_4k_va_buckets": page_count,
            "unique_64k_va_buckets": len(accumulator.pages_64k),
            ...
            "cache_set_count": _number_or_na(cache_set_count),
            "unique_cache_sets": _number_or_na(len(accumulator.cache_sets) if cache_set_count is not None else None),
            "page_occupancy_bytes_proxy": (unique_bytes / page_count) if page_count else 0.0,
            ...
            "local_line_revisit_references": accumulator.local_line_revisit_references if entry.order_model == "LOCAL_STREAM_ORDER" else "NA",
            "local_page_revisit_references": accumulator.local_page_revisit_references if entry.order_model == "LOCAL_STREAM_ORDER" else "NA",
...
VALIDATION_COLUMNS = [
    "manifest_sha256", "run_id", "window_ordinal", "trace_sha256", "capture_status", "terminal_status",
    "parse_status", "address_domain", "order_model", "sha256_verified", "object_boundary_policy", "result",
    "detail",
]
...
def validation_row(result: WindowResult) -> dict[str, str | int]:
    ...
    return {
        ...
        "object_boundary_policy": "FULL_ACCESS_SINGLE_LIVE_STORAGE_ELSE_UNKNOWN_RUNTIME",
...
REUSE_COLUMNS = [
    ...
    "current_window_ordinal", "object_class", "address_domain", "metric", "order_model", "intersection_count",
...
def reuse_rows(results: Iterable[WindowResult]) -> Iterator[dict[str, str | int | float]]:
    ...
    for (run_id, deployment_id, scenario_id, phase), windows in sorted(groups.items()):
        ...
        for previous, current in zip(windows, windows[1:]):
            classes = set(previous.object_lines) | set(current.object_lines) | set(OBJECT_CLASSES)
            for object_class in sorted(classes):
                for metric, prior_values, current_values in (
                    ("UNIQUE_128B_LINE_SET", previous.object_lines.get(object_class, set()), current.object_lines.get(object_class, set())),
                    ("UNIQUE_4K_VA_BUCKET_SET", previous.object_pages_4k.get(object_class, set()), current.object_pages_4k.get(object_class, set())),
                    ("UNIQUE_64K_VA_BUCKET_SET", previous.object_pages_64k.get(object_class, set()), current.object_pages_64k.get(object_class, set())),
                ):
...
def main() -> None:
    ...
    parser.add_argument("--cache-set-count", type=int, default=None,
                        help="optional modeled modulo set count; never interpreted as a measured hardware hash")
    args = parser.parse_args()
    if args.cache_set_count is not None and args.cache_set_count <= 0:
        parser.error("--cache-set-count must be positive")
    try:
        ...
        results = [fingerprint_entry(entry, manifest_sha, object_map, args.cache_set_count) for entry in entries]
        rows = [row for result in results for row in fingerprint_rows(result, args.cache_set_count)]
