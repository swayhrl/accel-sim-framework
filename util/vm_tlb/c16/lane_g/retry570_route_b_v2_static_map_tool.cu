...
static void emit_map(CUcontext context, CUfunction function, const Owner& owner) {
    ...
    // Persist the exact static instruction byte width alongside the MREF
    // cardinality. Q2 must freeze producer width from this actual NVBit map,
    // never infer it from opcode spelling.
