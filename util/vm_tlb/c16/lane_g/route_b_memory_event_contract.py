...
def validate_terminal(terminal: dict[str, Any], expected_event_count: int) -> None:
    """Require an unambiguous, complete terminal for exactly these events."""
    ...
    if terminal.get("event_count") != expected_event_count:
        raise RouteBContractError("Route-B producer terminal event_count differs from decoded LANE_EVENT count")
...
def validate_stream(events: Iterable[dict[str, Any]], whitelist: Iterable[WhitelistRow], terminal: dict[str, Any]) -> int:
    ...
    Each dynamic warp instruction is grouped by the producer's explicit
    ``(kernel_launch_id, warp_instruction_instance_id)`` identity.  The
    device buffer is reset at each launch, so its local instance counter is
    intentionally permitted to restart at zero.  This deliberately permits
    ...
    for event in events:
        ...
        instance = (event["kernel_launch_id"], event["warp_instruction_instance_id"])
