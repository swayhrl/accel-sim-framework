...
def parse_raw_jsonl(raw_path: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    cap = manifest.get("host_output_cap_bytes")
    if not isinstance(cap, int) or cap <= 0:
        raise RouteBContractError("Route-B producer manifest lacks a positive raw-byte cap")
    try:
        raw_bytes = raw_path.read_bytes()
    except OSError as exc:
        raise RouteBContractError(f"Route-B raw stream is unreadable: {raw_path}") from exc
    if len(raw_bytes) > cap:
        raise RouteBContractError("Route-B raw JSONL exceeds the authorized actual serialized-byte cap")
    try:
        lines = raw_bytes.decode("utf-8").splitlines()
    except UnicodeDecodeError as exc:
        raise RouteBContractError("Route-B raw JSONL is not UTF-8") from exc
    ...
    for line_number, line in enumerate(lines, 1):
        ...
        if record.get("record_kind") == "TERMINAL":
            if terminal_seen:
                raise RouteBContractError("Route-B raw stream has more than one terminal record")
            terminals.append(record)
            terminal_seen = True
        elif record.get("record_kind") == "LANE_EVENT":
            if terminal_seen:
                raise RouteBContractError("Route-B raw stream writes LANE_EVENT after terminal")
