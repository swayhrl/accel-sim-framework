...
and exact-function static-map evidence; it is never a timing or formal-trace
producer.
"""
...
MODES = ("NVBIT_LAUNCH_INVENTORY", "NVBIT_STATIC_MAP")
TERM_GRACE_S = 5
...
def child_command(args: argparse.Namespace, inventory: Path | None, static_map: Path | None) -> list[str]:
    command = [sys.executable, str(Path(__file__).with_name("nvbit_model_qualify.py")),
               ...
               "--mode", args.mode, "--tool-path", str(args.tool), "--tool-sha256", args.tool_sha256,
...
def main() -> None:
    ...
    static_map = args.raw_dir / "EXACT_FUNCTION_STATIC_MAP.tsv" if args.mode == "NVBIT_STATIC_MAP" else None
