...
struct WhitelistEntry {
    uint32_t static_index, mref_ordinal, instruction_offset, width_bytes, access_kind, mref_count;
    std::string opcode;
};
struct ContextState {
    ...
    // This is deliberately the number of JSON LANE_EVENTs already written,
    // not the device-local counter.  Device buffer sequences restart on each
    // launch, while the raw stream sequence must remain globally monotonic.
    uint64_t events_already_serialized = 0, total_overflow = 0, total_drop = 0, total_raw_bytes = 0;
...
static void load_whitelist() {
    ...
    // access_kind,opcode,memory_space,has_mref,mref_count.  The Python preflight manifest
    ...
    while (std::getline(input, line)) {
        ...
        if (field.size() != 9 || field[6] != "GLOBAL" || field[7] != "1") {
        ...
        WhitelistEntry entry{parse_u32(field[0], "static_index"), parse_u32(field[1], "mref_ordinal"),
                              ...
                              access_code(field[4]), parse_u32(field[8], "mref_count"), field[5]};
        if (entry.width_bytes == 0 || entry.mref_count == 0 || entry.mref_ordinal >= entry.mref_count ||
            !whitelist.emplace(std::make_pair(entry.static_index, entry.mref_ordinal), entry).second) {
...
static std::string event_json(const RouteBLaneEvent& e, const WhitelistEntry& entry, const ContextState* state) {
    ...
    // number of complete LANE_EVENTs already serialized into this JSONL file.
    out << "{\"record_kind\":\"LANE_EVENT\",\"raw_schema\":\"C16_ROUTE_B_LANE_EVENT_V1\","
        << "\"sequence_label\":\"OBSERVED_CALLBACK_ORDER\",\"observed_event_sequence\":"
        << state->events_already_serialized + e.observed_callback_sequence
        ...
        << ",\"opcode\":\"" << json_escape(entry.opcode) << "\",\"mref_ordinal\":" << e.mref_ordinal
...
static std::string terminal_json(bool complete, uint64_t overflow, uint64_t drop, uint64_t event_count) {
    std::ostringstream out;
    out << "{\"record_kind\":\"TERMINAL\",\"terminal_status\":\"" << (complete ? "COMPLETE" : "FAILED_CLOSED")
        << "\",\"overflow_count\":" << overflow << ",\"drop_count\":" << drop << ",\"event_count\":" << event_count << "}\n";
    return out.str();
}
static void append_readback(ContextState* state) {
    ...
    uint64_t invalid_records = 0, line_bytes = 0;
    for (uint64_t slot = 0; slot < count; ++slot) {
        ...
        if (e.record_kind != ROUTE_B_LANE_EVENT || found == whitelist.end()) { ++invalid_records; continue; }
        lines.push_back(event_json(e, found->second, state));
        line_bytes += lines.back().size();
    }
    const uint64_t candidate_overflow = state->total_overflow + state->host_buffer->overflow_count;
    const uint64_t candidate_drop = state->total_drop + state->host_buffer->drop_count + invalid_records;
    ...
    if (!out.good()) { fprintf(stderr, "C16_ROUTE_B_LANE_EVENT_RUNTIME_ERROR output write\n"); abort(); }
    state->events_already_serialized = candidate_events;
    state->total_raw_bytes += line_bytes;
