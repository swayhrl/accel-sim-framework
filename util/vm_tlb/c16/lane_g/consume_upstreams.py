...
def consume(out: Path) -> None:
    ...
    rows = [
        {"producer_lane": "A", "fixed_commit": A_COMMIT, "artifact_checkpoint_or_producer": str(a.get("artifact_checkpoint", "NA")), "manifest_path": f"{A_BASE}/PUBLISH_MANIFEST.json", "manifest_sha256": a_sha, "verified_file_count": str(a_count), "consumption_status": "HASH_VERIFIED_METADATA_INPUTS_ONLY", "dynamic_eligibility": "NO_GPU_PACKAGE_OR_TRANSFER_AUTHORIZATION", "reason": "inputs/scenarios and remote-weight hashes are fixed; C16_GPU_PACKAGE remains unpublished"},
    ...
    (out / "UPSTREAM_INPUT_AUDIT.md").write_text(
        ...
        f"A fixed commit `{A_COMMIT}` manifest verified {a_count} payload hashes. It supplies hash-bound model metadata, frozen input/token receipts, and scenarios, but remains a partial local-preparation publication: it does not authorize a GPU transfer or rental.\n\n"
