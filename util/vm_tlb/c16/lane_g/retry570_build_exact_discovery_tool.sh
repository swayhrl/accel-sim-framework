#!/usr/bin/env bash
# Build the no-insertion exact index-select discovery tool against frozen NVBit.
set -euo pipefail
...
PATH="/usr/local/cuda-12.4/bin:${PATH}" nvcc -std=c++17 -D_FORCE_INLINES -Xcompiler -fPIC -arch=sm_86 -O3 \
  -I"${release_root}/core" "${source_directory}/retry570_exact_discovery_only_tool.cu" \
  -L"${release_root}/core" -lnvbit -L/usr/local/cuda-12.4/lib64 -lcuda -lcudart_static -ldl -lrt -lpthread \
