...
static unsigned pick=0; static std::string target,out; static unsigned long long want=0,func_ord=0,global_ord=0,cap=0; static int cta_begin=0,cta_end=1; static C16Rec* rec=nullptr; static C16Ctr* ctr=nullptr; static bool installed=false;
...
void nvbit_at_cuda_event(CUcontext c,int exit,nvbit_api_cuda_t k,const char*,void*p,CUresult*){if(exit)return;CUfunction f;if(!launch(k,p,&f))return;global_ord++;if(target!=std::string(nvbit_get_func_name(c,f,true)))return;func_ord++;printf("C16_LOCC_MATCH global=%llu function=%llu selected=%d\n",global_ord,func_ord,func_ord==want);setup(c,f);nvbit_set_at_launch(c,f,global_ord);nvbit_enable_instrumented(c,f,func_ord==want);}
