...
QUALIFICATION = RUNTIME / 'observer_off_neutrality.json'
MODES = {
    'observer_off': {
        'label': 'OBSERVER_OFF',
    },
...
def qualified_targets() -> set[str]:
    if not QUALIFICATION.is_file():
        raise RuntimeError('observer_on requires observer_off_neutrality.json')
...
def run_point(target: str, mode: str, timeout: int,
    ...
    binary = BINARY
    core_lib = CORE_LIB
    if sha(binary) != '2cf75c70846a042d4154ef0ce5b24f755725e93e1cbe97442318f0fdd8c7ee04':
        raise RuntimeError('observer binary SHA mismatch')
    ...
    env['GPGPUSIM_AWMA_PASSIVE_MEMO_OBSERVER'] = \
        '1' if mode == 'observer_on' else '0'
    env['GPGPUSIM_AWMA_BOTTLENECK_OBSERVATORY'] = '0'
    ...
    receipt = {
        'stage': STAGE,
        'phase': 'INSTRUMENTATION_OFF_NEUTRALITY' if mode == 'observer_off'
                 else 'PASSIVE_OPPORTUNITY_OBSERVATION',
        'source_parent': 'b8a4064cb1bbe832758acf9dceb1461844c1fe4e',
        ...
        'observer_only': True, 'performance_mechanism_enabled': False,
        'argv': cmd,
...
def main() -> int:
    ...
    parser.add_argument('phase', choices=('observer_off', 'observer_on'))
    ...
    if args.phase == 'observer_on':
        passed = qualified_targets()
        missing = set(targets) - passed
        if missing:
            raise RuntimeError(
                f'observer_on forbidden for non-neutral targets: {sorted(missing)}')
