...
GATE_RECEIPT = RUNTIME / 'full_pipeline_gate.json'
GATE_BUNDLE = Path('/root/share/mnt164/huangrulin/c16_ai_workload/provenance/awma/crosscal_exact_v1r1_20260923T111500Z/pairs/M1/raw')
MODES = {
    'off': {
        'label': 'OFF',
    },
...
def qualified_targets() -> set[str]:
    ...
    for target in TARGETS:
        rc = DURABLE / f'{target}_OFF_10_80/rc.txt'
        if rc.is_file() and rc.read_text().strip() == '0':
            qualified.add(target)
    return qualified
...
def gate_passed() -> bool:
    if not GATE_RECEIPT.is_file():
        return False
    receipt = json.loads(GATE_RECEIPT.read_text())
    return receipt.get('stage') == STAGE and receipt.get('status') == 'PASS'
...
def run_point(target: str, mode: str, timeout: int,
    ...
    gate = target == 'ZERO_M1'
    if gate:
        payload = GATE_BUNDLE / 'kernel-2-ctx_0x58c3f6d68f70.traceg.xz'
        if sha(payload) != 'cb9e23a3ec9f6113051891053d22256a340f68524741faa986c9c00be7087bc8':
            raise RuntimeError('ZERO_M1 traceg SHA authority mismatch')
        index_sha = sha(GATE_BUNDLE / 'kernelslist.g')
    else:
        payload, index_sha = validate_input(target)
    binary = BINARY
    core_lib = CORE_LIB
    if sha(binary) != '84d4af7c3d4c94501261d62fff483b95625226508d80c1c3f5328cda3f527a83':
        raise RuntimeError('frozen V2R1 binary SHA mismatch')
    ...
    if gate:
        ...
        if traces.exists():
            shutil.rmtree(traces)
        traces.mkdir(parents=True)
        for name in ('kernel-1-ctx_0x58c3f6d68f70.traceg.xz',
                     'kernel-2-ctx_0x58c3f6d68f70.traceg.xz'):
            (traces / name).symlink_to(GATE_BUNDLE / name)
        shutil.copy2(GATE_BUNDLE / 'kernelslist.g', traces / 'kernelslist.g')
    else:
    ...
    env['GPGPUSIM_AWMA_V2R1_ACCOUNTING'] = '1'
    if mode == 'v2r1':
    ...
    env['GPGPUSIM_VM_COVERAGE_KERNEL_UID'] = '2' if gate else '1'
    ...
    receipt = {
        ...
        'family': 'CROSSCAL_M1_ZERO_OPPORTUNITY_DIRECTED' if gate else TARGETS[target]['family'],
        'performance_mechanism_enabled': mode == 'v2r1',
        'fixed_capacity': 1,
...
def main() -> int:
    ...
    parser.add_argument('phase', choices=('gate-off', 'gate-v2r1', 'off', 'v2r1'))
    ...
    gate = args.phase.startswith('gate-')
    mode = args.phase.removeprefix('gate-') if gate else args.phase
    targets = ('ZERO_M1',) if gate else (
        'T0', 'T1', 'T2', 'SPLITKV', 'COMBINE', 'A1', 'A2')
    if args.phase == 'v2r1':
        if not gate_passed():
            raise RuntimeError('V2R1 matrix forbidden before full-pipeline gate PASS')
        passed = qualified_targets()
        missing = set(targets) - passed
        if missing:
            raise RuntimeError(
                f'v2r1 forbidden for unqualified OFF targets: {sorted(missing)}')
    if args.phase == 'gate-v2r1':
        off_rc = DURABLE / 'ZERO_M1_OFF_10_80/rc.txt'
        if not off_rc.is_file() or off_rc.read_text().strip() != '0':
            raise RuntimeError('gate-v2r1 requires successful gate-off')
    DURABLE.mkdir(parents=True, exist_ok=True)
    results: list[dict[str, object]] = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = [pool.submit(run_point, target, mode,
