...
REPO = Path('/root/workspace/accel-sim-framework-awma-rtx4080-v1-ideal-translation-control-v1')
RUNTIME = Path('/root/awma_rtx4080_v1_ideal_translation_control_v1_runtime')
...
TARGETS = {
    'T0': {
        ...
        'index_content': 'kernel-34-ctx_0x5b5ba0bd1a60.traceg.xz\n',
    },
    'T1': {
    ...
    'T2': {
        ...
        'index_content': 'kernel-17039-ctx_0x5ddb6907c160.traceg.xz\n',
    },
}

EXPECTED = {
    'T0': {'instructions': 368696302, 'cta': 224, 'unique_uid': 3090304},
    'T1': {'instructions': 369131520, 'cta': 384, 'unique_uid': 7159808},
    'T2': {'instructions': 43357696, 'cta': 1216, 'unique_uid': 411008},
}
...
def run_point(target: str, timeout_s: int) -> dict[str, object]:
    run_dir = RUNTIME / f'ai_{target}_V1_ideal'
    ...
    authority = {
        'target': target, 'semantic': 'V1_IDEAL_TRANSLATION_CONTROL', 'latency': 'ideal',
...
def main() -> int:
    ...
    with authority.open('w', newline='') as f:
        ...
        for (target,) in points:
            payload = TARGETS[target]['payload']
            index = str(TARGETS[target]['index_content'])
            w.writerow([target, 'V1_IDEAL_TRANSLATION_CONTROL', 'ideal', payload, sha_bytes(payload.read_bytes()),
                        sha_bytes(index.encode()), config_sha, trace_config_sha, binary_sha,
                        1, 0, RUNTIME / f'ai_{target}_V1_ideal',
