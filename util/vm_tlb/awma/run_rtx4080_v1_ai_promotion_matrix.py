...
def run_point(target: str, semantic: str, latency: int, timeout_s: int) -> dict[str, object]:
    run_dir = RUNTIME / f'ai_{target}_{semantic}_{latency}_80'
    ...
    authority = {
        'target': target, 'semantic': semantic, 'latency': f'{latency}/80',
...
def main() -> int:
    ...
    args = ap.parse_args()
    points = [(t, s, l) for t in ('T0', 'T1', 'T2') for s in ('LEGACY', 'V1') for l in (10, 0)]
    ...
    with authority.open('w', newline='') as f:
        ...
        for target, semantic, latency in points:
            payload = TARGETS[target]['payload']
            index = str(TARGETS[target]['index_content'])
            w.writerow([target, semantic, f'{latency}/80', payload, sha_bytes(payload.read_bytes()),
                        sha_bytes(index.encode()), config_sha, trace_config_sha, binary_sha,
                        1 if semantic == 'V1' else 0, 0,
                        RUNTIME / f'ai_{target}_{semantic}_{latency}_80',
