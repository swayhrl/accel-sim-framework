#!/usr/bin/env bash
# Future-only strict collector for the live ATAX FAST64.3 Base precompute.
...
row=fast64_3_precomputed_atax_base_cap8192_a1_r2
run="$runs/$row"
config="$repo/configs/dtc_l1/fast64/FAST64_BASE.config"
out="$repo/docs/dtc_l1/fast64/generated/fast64_3_atax_base_alias_v2"
...
"$validator" --run-dir "$run" --workload-id ATAX --mode BASE \
...
if rg -n -i 'assert|fatal|deadlock|output mismatch|checker.*fail' \
  ...
  echo FAST64_3_ATAX_FORBIDDEN_FAILURE_SIGNATURE >&2
  exit 1
fi

python3 - "$out/$row.json" "$out/FAST64_3_ATAX_BASE_ALIAS_V2.tsv" <<'PY'
import json
import pathlib
import sys
...
if missing:
    raise SystemExit("missing strict ATAX Base metrics: " + ", ".join(missing))
...
evidence.write_text(
    "schema\tFAST64_3_ATAX_BASE_ALIAS_V2\n"
...
echo FAST64_3_ATAX_BASE_ALIAS_V2_COLLECTED
