#!/usr/bin/env bash
# Future-only, read-only closeout monitor for a repaired-Core FAST64 row.
# v1 is retained unchanged.  v2 fixes only optional host launcher-log scanning.
set -euo pipefail
...
test ! -e "$output" || { echo "FAST64_REPAIRED_CORE_V2_OUTPUT_EXISTS $output" >&2; exit 1; }

lock="${output}.lock"; mkdir -p "$(dirname "$output")"
exec 9>"$lock"; flock -n 9 || { echo FAST64_REPAIRED_CORE_V2_MONITOR_ALREADY_ACTIVE >&2; exit 1; }
emit() { printf '%s\tutc=%s\t%s\n' "$1" "$(date -u +%FT%TZ)" "$2" | tee -a "$log"; }
while ! test -f "$run/RUN_TERMINAL.tsv"; do
  emit FAST64_REPAIRED_CORE_V2_WAIT_TERMINAL "workload=$workload mode=$mode run=$run"
  sleep "$poll"
done
exit_status=$(awk -F '\t' '$1=="simulator_exit_status" {n++;v=$2} END {if(n==1)print v;else exit 1}' "$run/RUN_TERMINAL.tsv")
test "$exit_status" = 0 || { emit FAST64_REPAIRED_CORE_V2_NONZERO_TERMINAL "workload=$workload mode=$mode"; exit 1; }
emit FAST64_REPAIRED_CORE_V2_VALIDATE "workload=$workload mode=$mode run=$run"
...
if test -f "$run/launcher.log"; then scan_files+=("$run/launcher.log"); fi
if rg -n -i 'assertion failed|fatal error|deadlock detected|output mismatch|checker.*fail|segmentation fault|core dumped' \
    "${scan_files[@]}"; then
  emit FAST64_REPAIRED_CORE_V2_FORBIDDEN_FAILURE_SIGNATURE "workload=$workload mode=$mode run=$run"
  exit 1
fi
emit FAST64_REPAIRED_CORE_V2_PASS "workload=$workload mode=$mode output=$output"
