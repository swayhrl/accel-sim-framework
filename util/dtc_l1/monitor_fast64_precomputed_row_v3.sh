#!/usr/bin/env bash
# Future-only closeout monitor for one immutable FAST64 precompute.
#
# v2 remains frozen for its live consumers.  This revision changes only the
# host-side optional-log scan: a valid run has simulator.stdout/stderr, while
# simulator.launcher.log is not emitted by every immutable runner invocation.
# It does not change any simulator input, receipt, parser, or result meaning.
set -euo pipefail
...
test ! -e "$output" || { echo "FAST64_PRECOMPUTED_ROW_V3_OUTPUT_EXISTS $output" >&2; exit 1; }

lock="${output}.lock"
mkdir -p "$(dirname "$output")"
exec 9>"$lock"
flock -n 9 || { echo FAST64_PRECOMPUTED_ROW_V3_MONITOR_ALREADY_ACTIVE >&2; exit 1; }
emit() { printf '%s\tutc=%s\t%s\n' "$1" "$(date -u +%FT%TZ)" "$2" | tee -a "$log"; }

while ! test -f "$run/RUN_TERMINAL.tsv"; do
  emit FAST64_PRECOMPUTED_ROW_V3_WAIT_TERMINAL "workload=$workload mode=$mode run=$run"
  sleep "$poll"
done

value() { awk -F '\t' -v k="$2" '$1==k {n++;v=$2} END {if(n==1)print v;else exit 1}' "$1"; }
test "$(value "$run/RUN_TERMINAL.tsv" simulator_exit_status)" = 0 || {
  emit FAST64_PRECOMPUTED_ROW_V3_NONZERO_TERMINAL "workload=$workload mode=$mode run=$run"; exit 1;
}
emit FAST64_PRECOMPUTED_ROW_V3_VALIDATE "workload=$workload mode=$mode run=$run"
...
# runner launch logs are optional; required simulator logs are always scanned.
scan_files=("$run/simulator.stdout" "$run/simulator.stderr")
if test -f "$run/simulator.launcher.log"; then scan_files+=("$run/simulator.launcher.log"); fi
if rg -n -i 'assertion failed|fatal error|deadlock detected|output mismatch|checker.*fail|segmentation fault|core dumped' \
    "${scan_files[@]}"; then
  emit FAST64_PRECOMPUTED_ROW_V3_FORBIDDEN_FAILURE_SIGNATURE "workload=$workload mode=$mode run=$run"
  exit 1
fi
emit FAST64_PRECOMPUTED_ROW_V3_PASS "workload=$workload mode=$mode output=$output"
