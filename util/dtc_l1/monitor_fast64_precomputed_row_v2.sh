...
#
# v1 is retained for its live users.  This version fixes only the host-side
# invocation of the non-executable Python validator; it does not change
# simulator inputs, receipt semantics, parsing policy, or result meaning.
set -euo pipefail
...
test ! -e "$output" || { echo "FAST64_PRECOMPUTED_ROW_V2_OUTPUT_EXISTS $output" >&2; exit 1; }

lock="${output}.lock"
mkdir -p "$(dirname "$output")"
exec 9>"$lock"
flock -n 9 || { echo FAST64_PRECOMPUTED_ROW_V2_MONITOR_ALREADY_ACTIVE >&2; exit 1; }
emit() { printf '%s\tutc=%s\t%s\n' "$1" "$(date -u +%FT%TZ)" "$2" | tee -a "$log"; }

while ! test -f "$run/RUN_TERMINAL.tsv"; do
  emit FAST64_PRECOMPUTED_ROW_V2_WAIT_TERMINAL "workload=$workload mode=$mode run=$run"
  sleep "$poll"
done

value() { awk -F '\t' -v k="$2" '$1==k {n++;v=$2} END {if(n==1)print v;else exit 1}' "$1"; }
test "$(value "$run/RUN_TERMINAL.tsv" simulator_exit_status)" = 0 || {
  emit FAST64_PRECOMPUTED_ROW_V2_NONZERO_TERMINAL "workload=$workload mode=$mode run=$run"; exit 1;
}
emit FAST64_PRECOMPUTED_ROW_V2_VALIDATE "workload=$workload mode=$mode run=$run"
...
if rg -n -i 'assertion failed|fatal error|deadlock detected|output mismatch|checker.*fail|segmentation fault|core dumped' \
  ...
  emit FAST64_PRECOMPUTED_ROW_V2_FORBIDDEN_FAILURE_SIGNATURE "workload=$workload mode=$mode run=$run"
  exit 1
fi
emit FAST64_PRECOMPUTED_ROW_V2_PASS "workload=$workload mode=$mode output=$output"
