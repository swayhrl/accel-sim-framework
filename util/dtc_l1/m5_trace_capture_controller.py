...
def record(a,s,src,script,cmd,exe,inv,t,raw,grp,prov,device):
 ...
 rh=setsha(raw,t);gh=setsha(grp,t);bid=hashlib.sha256((s.workload_id+rh+gh+sha(t/"kernelslist.g")).encode()).hexdigest();d=t.parent;repair={"kind":"NONE"}
 if s.workload_id=="gesu":
  rp=exe.parent/"gesummv.m5_source_repair.json"
  try:repair=json.loads(rp.read_text())
  except (OSError,json.JSONDecodeError) as e:raise RuntimeError("GESUMMV source-repair provenance required") from e
  if repair.get("kind")!="GESUMMV_ZERO_DEVICE_ACCUMULATORS" or repair.get("original_source_sha256")!=sha(q) or not repair.get("prepared_source_sha256"):raise RuntimeError("invalid GESUMMV source-repair provenance")
 return {"trace_bundle_id":bid,"workload":s.workload_id,"canonical_id":s.canonical_id,"source_kind":s.source_kind,"source_commit":s.source_commit,"source_tree_sha":source["tree"],"source_file":s.source_file,"source_file_sha":sha(q),"source_repair":repair,"source_dependencies":deps,"input_identity_kind":s.input_kind,"input_hashes":inp,"build_script":s.build_script,"build_script_sha":sha(script),"build_command":cmd,"trace_capture_binary_sha":sha(exe),"checker_kind":s.checker_kind,"checker_reference_sha":inp.get("reference_sha256","N/A"),"gpu_uuid_model_cc":device,"tracer_source_tree_sha":prov["tracer_source_tree_sha"],"tracer_tool_sha":prov["tracer_tool_sha"],"nvbit_archive_sha":prov["nvbit_archive_sha"],"postprocess_sha":prov["postprocess_sha"],"trace_format":"NVBit-v1.8","kernel_invocation_count":len(inv),"kernel_invocation_manifest_sha":sha(d/"kernel_invocation_manifest.json"),"kernel_geometry_manifest_sha":sha(d/"kernel_geometry_manifest.json"),"kernelslist_sha":sha(t/"kernelslist"),"kernelslist_g_sha":sha(t/"kernelslist.g"),"raw_trace_count":len(raw),"grouped_trace_count":len(grp),"raw_trace_set_sha256":rh,"traceg_set_sha256":gh}
