#!/usr/bin/env bash
# Future-only immutable dispatch for repaired-Core FAST64 acquisition.
#
# v4 resolves the mutable shared Core worktree dependency in v3.  It pins the
# existing detached Core95 formal worktree, leaving the shared Core41 repair
# worktree and every live controller untouched.
set -euo pipefail
...
runs=/workspace/fast64-repaired-ramp3
# This detached worktree is the sole source-tree identity witness for Core95.
# It must not be replaced with the shared Core worktree, which may legitimately
# advance for a different, already-live repair diagnostic.
...
if [ "$dry_run" = 1 ]; then
  printf 'FAST64_REPAIRED_CORE_V4_DISPATCH_DRY_RUN_PASS\tname=%s\tworkload=%s\tmode=%s\tcpu=%s\tclassification=%s\n' \
    "$name" "$workload" "$mode" "$cpu" "$classification"
  exit 0
fi

mkdir -p "$runs"
exec 9>"$runs/.fast64_repaired_core_v4_dispatch.lock"
flock -n 9 || { echo "FAST64_REPAIRED_CORE_V4_DISPATCH_LOCK_HELD" >&2; exit 1; }
for p in "$run" "$run.launcher.log" "$run.supervisor.tsv"; do
...
printf 'FAST64_REPAIRED_CORE_V4_DISPATCHED\trow=%s\tcpu=%s\tsupervisor=%s\tattempt_uuid=%s\n' \
