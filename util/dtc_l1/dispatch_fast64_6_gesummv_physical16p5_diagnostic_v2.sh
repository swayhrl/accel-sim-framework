#!/usr/bin/env bash
# Future-only, nonformal GESUMMV 16.5-KiB diagnostic.  V2 retains V1's exact
# observational Core/binary contract but validates it in a detached f283
# worktree, so the active Core-41 worktree remains untouched.
set -euo pipefail
...
core=/workspace/worktrees/gpgpu-sim-fast64-diag-f283
...
run=/workspace/fast64-diagnostics/fast64_6_gesummv_physical16p5_${mode,,}_coref283_diag_v2
...
if test "$dispatch" = 0; then
  printf 'FAST64_6_GESUMMV_DIAG_V2_DRY_RUN_PASS\tmode=%s\tcore=%s\tbinary=%s\tconfig=%s\n' "$mode" "$core_sha" "$sim_sha" "$config_sha"
  exit 0
fi
