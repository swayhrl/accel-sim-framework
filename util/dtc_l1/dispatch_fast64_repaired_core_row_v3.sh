...
#
# This intentionally does not replace dispatch_fast64_precomputed_row_v2.sh:
# that historical helper is pinned to the bbcbb telemetry runtime.  Every row
# created here carries the post-zero-access-repair Core/runtime authority.
set -euo pipefail
...
if [ "$dry_run" = 1 ]; then
  printf 'FAST64_REPAIRED_CORE_V3_DISPATCH_DRY_RUN_PASS\tname=%s\tworkload=%s\tmode=%s\tcpu=%s\tclassification=%s\n' \
    "$name" "$workload" "$mode" "$cpu" "$classification"
  exit 0
fi

mkdir -p "$runs"
exec 9>"$runs/.fast64_repaired_core_v3_dispatch.lock"
flock -n 9 || { echo "FAST64_REPAIRED_CORE_V3_DISPATCH_LOCK_HELD" >&2; exit 1; }
for p in "$run" "$run.launcher.log" "$run.supervisor.tsv"; do
...
printf 'FAST64_REPAIRED_CORE_V3_DISPATCHED\trow=%s\tcpu=%s\tsupervisor=%s\tattempt_uuid=%s\n' \
