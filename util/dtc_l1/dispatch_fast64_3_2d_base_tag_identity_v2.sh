#!/usr/bin/env bash
# Future-only immutable dispatcher for the 2DConvolution/Base tag-identity
# repair.  It is deliberately separate from every Core-41/live controller.
set -euo pipefail
...
run_root=/workspace/fast64-stage3-tag-identity-repair-v2
row=fast64_3_2DConvolution_base_core6587238c_a1_v1
run="$run_root/$row"
...
if test "$dispatch" = 0; then
  printf 'FAST64_3_2D_TAG_IDENTITY_V2_DRY_RUN_PASS\tcore=%s\truntime=%s\tcpu=%s\n' \
    "$core_sha" "$runtime_sha" "$cpu"
  exit 0
fi

mkdir -p "$run_root"
exec 9>"$run_root/.fast64_3_2d_tag_identity_v2_dispatch.lock"
...
printf 'row\tsupervisor_pid\tcpu_slot\tattempt_uuid\trunner_sha256\timmutable_runner\tcore_sha\truntime_sha256\tclassification\n%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
  "$row" "$supervisor" "$cpu" "$uuid" "$runner_sha" "$runner" "$core_sha" \
  "$runtime_sha" "$classification" >"$run.supervisor.tsv"
printf 'FAST64_3_2D_TAG_IDENTITY_V2_DISPATCHED\trow=%s\tcpu=%s\tsupervisor=%s\tattempt_uuid=%s\n' \
