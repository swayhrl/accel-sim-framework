#!/usr/bin/env bash
# Future-only strict collector for the GEMM/Base immutable-v2 precompute.
# It is deliberately separate from the live v3 monitor and its three-row
# scope, and from every frozen FAST64.1 R2 closeout dependency.
set -euo pipefail
...
out="$repo/docs/dtc_l1/fast64/generated/fast64_3_gemm_base_alias_v4"
...
# Match actual diagnostics, never the normal -gpgpu_deadlock_detect config echo.
if rg -n -i 'assertion failed|assert\(|fatal error|deadlock detected|output mismatch|checker.*fail|segmentation fault|core dumped' \
  ...
  echo FAST64_3_GEMM_FORBIDDEN_FAILURE_SIGNATURE >&2
  exit 1
fi

python3 - "$out/$row.json" "$out/FAST64_3_GEMM_BASE_ALIAS_V4.tsv" <<'PY'
import json
import pathlib
import sys
...
echo FAST64_3_GEMM_BASE_ALIAS_V4_COLLECTED
