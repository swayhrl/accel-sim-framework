#!/usr/bin/env bash
# Future-only immutable-v2 launch for the source-backed FAST64.2 NN/IO
# coupled diagnostic.  Cap 1 and IO candidate entries 1 are diagnostic-only.
set -euo pipefail
...
case "${1:-}" in
  ''|--dry-run) [ "$#" -le 1 ] || usage ;;
  --launch) mode=launch; test "${2:-}" = --resource-audit && audit=${3:-} && [ "$#" = 3 ] || usage ;;
  *) usage ;;
esac
...
run="$runs/$row"
classification=FAST64_2_SOURCE_REACHABLE_COUPLED_STRESS_CAP1_DIAGNOSTIC
...
trace_root=$(awk -F '\t' '$1=="nn" {print $2;exit}' "$repo/docs/dtc_l1/fast64/generated/FAST64_PAYLOAD_MANIFEST.tsv")
test -n "$trace_root" && test -r "$trace_root/kernelslist.g"
for p in "$run" "$run.launcher.log" "$run.supervisor.tsv"; do test ! -e "$p" || { echo "FAST64_2_CAP1_TARGET_EXISTS $p" >&2; exit 1; }; done
cpu=$(python3 "$repo/util/dtc_l1/select_fast64_r2_cpus.py" --format tsv | awk -F '\t' '$1=="candidate_cpus" {split($2,a,",");print a[1];exit}')
test -n "$cpu"

if [ "$mode" = dry-run ]; then
  printf 'FAST64_2_CAP1_DRY_RUN_PASS\trow=%s\tcpu=%s\trunner_sha256=%s\n' "$row" "$cpu" "$runner_sha"
  exit 0
fi
test -r "$audit"
awk -F '\t' '$1=="schema"&&$2=="FAST64_FUTURE_PRECOMPUTE_RESOURCE_AUDIT_V1"{s=1}$1=="safe_to_launch"&&$2=="YES"{y=1}$1=="authorized_workers"&&$2=="1"{w=1}$1=="swap_so_delta"&&$2=="0"{a=1}$1=="oom_kill_delta"&&$2=="0"{b=1}$1=="memory_psi_avg10"&&($2=="0"||$2=="0.00"){c=1}END{exit !(s&&y&&w&&a&&b&&c)}' "$audit" || { echo FAST64_2_CAP1_AUDIT_UNSAFE >&2; exit 1; }
exec 9>"$runs/.fast64_2_cap1_v1_dispatch.lock"
flock -n 9 || { echo FAST64_2_CAP1_DISPATCH_LOCK_HELD >&2; exit 1; }
for p in "$run" "$run.launcher.log" "$run.supervisor.tsv"; do test ! -e "$p" || { echo "FAST64_2_CAP1_TARGET_RACED $p" >&2; exit 1; }; done
...
printf 'FAST64_2_CAP1_DISPATCHED\trow=%s\tcpu=%s\tsupervisor=%s\tattempt_uuid=%s\n' "$row" "$cpu" "$supervisor" "$uuid"
