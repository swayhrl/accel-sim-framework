#!/usr/bin/env bash
# Future-only Core-658 2DConvolution IO/OO dispatcher.  This is identical to
# v2 except that it consumes the V2 structural companion, whose source_summary
# points to the final atomically-published Base JSON rather than the v2
# collector's temporary path.
set -euo pipefail
...
if test "$dispatch" = 0; then
  printf 'FAST64_4_2D_TAG_IDENTITY_V3_DRY_RUN_PASS\tmode=%s\tcore=%s\truntime=%s\tcpu=%s\n' "$mode" "$core_sha" "$runtime_sha" "$cpu"
  exit 0
fi
mkdir -p "$run_root"
exec 9>"$run_root/.fast64_4_2d_tag_identity_v3_dispatch.lock"
...
printf 'FAST64_4_2D_TAG_IDENTITY_V3_DISPATCHED\trow=%s\tmode=%s\tcpu=%s\tsupervisor=%s\tattempt_uuid=%s\n' \
