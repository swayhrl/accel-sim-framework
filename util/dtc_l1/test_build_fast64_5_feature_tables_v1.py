...
with tempfile.TemporaryDirectory() as d:
    ...
    ledger=d/"ledger.tsv";tsv(ledger,["stage","status"],[{"stage":"FAST64.4","status":"PASS"}])
    ...
    tsv(ledger,["stage","status"],[{"stage":"FAST64.4","status":"ACTIVE"}]);bad=subprocess.run([str(TOOL),"--stage-ledger",str(ledger),"--stage4-dir",str(s4),"--stage3-dir",str(s3),"--output-dir",str(d/"bad")],text=True,capture_output=True);assert bad.returncode!=0 and "PASS_REQUIRED" in bad.stderr
