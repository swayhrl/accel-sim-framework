#!/usr/bin/env bash
# Future-only strict collector for the live GESUMMV FAST64.3 Base precompute.
# It never launches, waits on, signals, or changes the run directory.  Its
# successful collection remains pending until FAST64.1/2 logical acceptance.
set -euo pipefail

repo=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
runs=/workspace/fast64-runs
row=fast64_3_precomputed_gesummv_base_cap8192_a1_r2
run="$runs/$row"
config="$repo/configs/dtc_l1/fast64/FAST64_BASE.config"
out="$repo/docs/dtc_l1/fast64/generated/fast64_3_gesummv_base_alias_v2"
...
"$validator" --run-dir "$run" --workload-id GESUMMV --mode BASE \
...
if rg -n -i 'assert|fatal|deadlock|output mismatch|checker.*fail' \
    "$run.launcher.log" "$run/simulator.stdout" "$run/simulator.stderr"; then
  echo FAST64_3_GESUMMV_FORBIDDEN_FAILURE_SIGNATURE >&2
  exit 1
fi

python3 - "$out/$row.json" "$out/FAST64_3_GESUMMV_BASE_ALIAS_V2.tsv" <<'PY'
import json
import pathlib
import sys
...
if missing:
    raise SystemExit("missing strict GESUMMV Base metrics: " + ", ".join(missing))
...
echo FAST64_3_GESUMMV_BASE_ALIAS_V2_COLLECTED
