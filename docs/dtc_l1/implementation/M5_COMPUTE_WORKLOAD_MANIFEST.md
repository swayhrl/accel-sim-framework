...
| `gesu` | `RATIO0_BASE_OUTPUT_CLEAN_STRICT` | Corrected GESUMMV recovery completed naturally with zero source CPU/GPU mismatches, final PIB/lower occupancy zero, exact `4194688/4194688` PIB admit/retire and `34327761/34327761` lower acquire/release closure. Registry `M5-e648006f1b9cf5f4` and `m5/generated/m5_0b_gesummv_base_ratio0.json` preserve the strict result. |
| `gemv` | `RATIO0_BASE_OUTPUT_CLEAN_STRICT` | The source-equivalent GEMVER recovery completed naturally with its source-defined zero-miss verdict. Strict parsing closes final PIB/lower occupancy at zero, exact `6292096/6292096` PIB admit/retire and `22065010/22065010` lower acquire/release. Registry `M5-f24650643302f0ac` and `m5/generated/m5_0b_gemver_base_ratio0.json` preserve the result under canonical identity `polybench_gemver_4096`. |
| remaining six | `ACTIVE_FORMAL_BASE_16KB` | Corrected isolated Base runs for ATAX, MVT, SYRK, SYR2K, 2MM, and Conv2D are active; no output claim is made before source self-check and strict invariant parsing complete. |
