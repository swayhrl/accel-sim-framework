...
immutable wave. Gaussian Base/IO/OO, LUD Base/IO/OO, and Core658 2D IO are
all non-binding at 16384. Hotspot1 IO/OO remain bound at 16384 (`25,887` /
`23,026` cap-full events), but both are non-binding at 32768 (`0` events).
Thus 32768 is the minimum common cap, not a speedup-selected value. The
SHA-bound 36-cell map admits only explicit source-proven inert reuse or final
cap reacquisition; the fail-closed collector passes all 12 triplets, all
accounting/drain conditions, and exact FAST12 membership. GM-FAST12 is
`1.326143` (IO) / `1.592062` (OO). `FAST64_4_PRIMARY_PASS` is closed.
