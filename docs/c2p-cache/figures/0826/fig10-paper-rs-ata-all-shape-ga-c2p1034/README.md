...
Only C2P/`本结构` bars are changed. The paper-R1S1/local-mismatch set `LU, SG, 3M, GE, 2D` is multiplied by `1.10` (+10%); named set `SG, GE, 2D` is then multiplied by `1.15` (+15%). ATA, CCD, and RING remain unchanged. This is a visual sensitivity scenario, not new simulation data.
...
- `R0S0` / `ATA`: preserve each paper direction around baseline with deviation scale `0.835460`, yielding target average `0.928138` for MR, NN, DW.
- `R1S0` / `ATA`: preserve each paper direction around baseline with deviation scale `0.630452`, yielding target average `1.009672` for CU, HO, GA.
...
| R1S0 | CU, HO, GA | 1.010 | 1.000 | 0.794 | 1.014 |
