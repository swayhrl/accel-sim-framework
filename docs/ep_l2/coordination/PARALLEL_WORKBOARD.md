...
| L1-D256-META | L1 causality | L1 metadata/queue headroom at D256 | Keep 64KiB/tag geometry/latency/banks fixed; MSHR 512->1024, per-line merge 8->32, MissQ 16->64 | vectorAdd, scan, spmv, convolution, btree, sad, FWT_7_21 | current formal D256 base exists | DONE | 7/7 B0-Banked rows are `COMPLETE_VALID`, including `scan` (2,151,187 cycles); provenance/config audit and parser/invariants pass. C8 full screen finds no >5% META-HR performance response or strong downstream trigger; no C9 decomposition is required. | Framework `hrl/ep-l2-l1-causality-v0` @ `dc30e67`; Core `ece1a3a77c5628763e0a4605bfd1c639ee6a1495`; V2 contract `calibration/contracts/D256_META_HR.json` | PASS | Accepted input to `L1_CAUSALITY_SCREEN_COMPLETE`; primary L1 baseline unchanged. |
| L1-D256-BANK | L1 causality | L1 bank-throughput headroom at D256 | Keep capacity/tag/MSHR/MissQ fixed; banks 4->8 only | same 7 | current formal D256 base exists | DONE | 7/7 B0-Banked rows are `COMPLETE_VALID`, including `scan` (2,160,489 cycles); provenance/config audit, parser and invariants pass. C8 full screen has no material META-HR trigger requiring C9 decomposition. | Framework `hrl/ep-l2-l1-causality-v0` @ `dc30e67`; Core `ece1a3a77c5628763e0a4605bfd1c639ee6a1495`; V2 contract `calibration/contracts/D256_BANK_HR.json` | PASS | Accepted input to `L1_CAUSALITY_SCREEN_COMPLETE`; primary L1 baseline unchanged. |
| L1-D512-META | Interaction | L1 metadata headroom after descriptor relief | D512 + same L1 metadata headroom | same 7 | `D256_EQ_SCAN_PASS` + `D512_PREFLIGHT_PASS` | DONE | 7/7 B0-Banked rows promoted without rerun from `SPECULATIVE_PENDING_GATE` to `PROMOTED_VALID_CALIBRATION`; exact parent Core `878f80869ce212e779df20b6421e4dc7f987825d` / Framework `aae62b66685f15437cecf0193934f628e6fac6ae`; promotion manifest binds workboard `454264f`. | `/workspace/results/ep_l2_l1_causality_d512_speculative/D512-META-HR/`; V2 contract `calibration/contracts/D512_META_HR.json` | PASS | Accepted input to `L1_CAUSALITY_SCREEN_COMPLETE`; no primary-L1 change. |
| L1-D512-BANK | Interaction | L1 bank headroom after descriptor relief | D512 + banks 4->8 | same 7 | `D256_EQ_SCAN_PASS` + `D512_PREFLIGHT_PASS` | DONE | 7/7 B0-Banked rows promoted without rerun from `SPECULATIVE_PENDING_GATE` to `PROMOTED_VALID_CALIBRATION`; same exact parent and promotion manifest. | `/workspace/results/ep_l2_l1_causality_d512_speculative/D512-BANK-HR/`; V2 contract `calibration/contracts/D512_BANK_HR.json` | PASS | `L1_CAUSALITY_SCREEN_COMPLETE`; no primary-L1 change. |
| CAL-ANALYSIS | Calibration analysis | Joint D256/D512 x L1 causality analysis | Compare cycles, L1 events, L2 descriptor/MSHR, lower traffic, scheduler/BW, and temporal windows | selected + full D512 | all required calibration runs | DONE | Final convergence r1 complete: 13-workload archetype checkpoint; 80-row six-cell provenance-bound matrix (26+26+4×7); Lane-E MSHR 2×2 separate supplement; D512/L1/Line-MSHR evidence synthesized into recommendation-only baseline/mechanism map. No simulator run or B/C/E result-root modification. | `review_packs/FINAL_CALIBRATION_CONVERGENCE_r1/`; source `hrl/ep-l2-cal-analysis-v0`; `codex_handoff/LANE_D_LATEST.md` | PENDING | Request independent ChatGPT final-convergence review. This is not BASELINE-DECISION_PASS and does not authorize functional mechanism work. |
