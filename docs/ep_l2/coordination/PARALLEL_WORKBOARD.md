...
| MSHR256-AUDIT | Lane E | Audit/generalize Line-MSHR256 support | Keep descriptor semantics fixed; ensure allocator + line occupancy telemetry/p95/window state represent >128 exactly; prove MSHR128 equivalence if source changes | directed + vectorAdd + convolution | frozen Lane-B candidate | DONE | Allocator/full reason is config-driven; 1025-bin line histogram plus p95/delta/parser cover >128. New 127/128/129 and 255/256 test passes; D512 MSHR128 vectorAdd and convolution each have seven parsed CSV artifacts byte-identical to Lane B. | Lane-E Core `d1f51667` (semantic parent `878f8086`); Framework `e6f1b2d` (semantic parent `aae62b66`); `review_packs/LINE_MSHR_CAUSALITY_r1/` | PENDING | Observation/parameterization-only changes only; exact MSHR128 equivalence required. |
| MSHR256-2X2 | Lane E | Descriptor × Line-MSHR causal 2x2 on convolution | Existing D256/M128 + D512/M128; new D256/M256 + D512/M256; B0-Banked only | convolutionSeparable | MSHR256-AUDIT; D512 child promotion waits for `D512_PREFLIGHT_PASS` | RUNNING | Local execution is `COMPLETE_VALID`: D256 M128/M256 both 290,308 cycles; D512 M128/M256 = 292,211/291,108 cycles. D512 Line-MSHR-full 931,416->0 but speedup is 0.38%; local classification `MSHR_ADMISSION_THROTTLE_DOWNSTREAM_LIMITED`. D512/M256 remains `SPECULATIVE_PENDING_GATE`. | `/workspace/results/ep_l2_line_mshr_causality/`; `review_packs/LINE_MSHR_CAUSALITY_r1/CONVOLUTION_2X2.csv` | PENDING | Determine whether the 931,416 D512 Line-MSHR-full blocks are performance-causal or downstream-limited. |
| MSHR256-CONTROL | Lane E | Near-threshold negative control | D512 spmv MSHR128->256 only; MSHR128 has max125 but exact full-block=0 | spmv | MSHR256-AUDIT; promotion waits for `D512_PREFLIGHT_PASS` | RUNNING | Local execution is `COMPLETE_VALID`; `SPECULATIVE_PENDING_GATE`. Both MSHR128 and MSHR256 are exactly 23,560 cycles with matching parsed counters. | `/workspace/results/ep_l2_line_mshr_causality/`; `review_packs/LINE_MSHR_CAUSALITY_r1/SPMV_NEGATIVE_CONTROL.csv` | PENDING | Material response would weaken a simple exact-full-block causal interpretation and requires investigation. |
