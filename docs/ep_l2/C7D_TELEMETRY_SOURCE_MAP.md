...
| Descriptor chain depth | `mshr_table` target-list ownership | Existing production max is retained; histogram extension is a review gap until sourced from per-entry target-list state. |
...
| DRAM scheduler/return path | Native `dram_t` and FR-FCFS statistics | `block_lower` is never interpreted as scheduler blocking; unconnected fields stay unavailable. |
