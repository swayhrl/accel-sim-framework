...
| M1 | `src/gpgpu-sim/gpu-cache.h`, `gpu-cache.cc`, `mem_fetch.h` only if handle naming needs extension, `gpu-sim.cc` for independent feature-bit/static-policy parsing and fail-closed validation | `tests/ep_l2/test_payload_store.cc`, `test_payload_banked.cc`, new lifecycle/equivalence/config-parser test, small overlays and config/docs |
| M2 | same M1 core files; `l2cache.cc` only for observation/reporting | shared-pool forward-progress/OFF-vs-ON tests, deterministic mode overlay/manifest runner support, review pack parser/docs |
