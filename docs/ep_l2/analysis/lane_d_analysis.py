...
def channel_imbalance(records: list[dict[str, str]], traffic_fraction_threshold: float = .01) -> dict[str, float | str]:
    ...
    for row in records:
        ...
        if value is not None and denominator is not None:
            groups[row["window_start_cycle"]].append((value, denominator))
    ratios, cvs, active_counts, conditioned = [], [], [], []
    for pairs in groups.values():
        values = [value for value, _ in pairs]
        mean = sum(values) / len(values)
        total, capacity = sum(values), sum(denominator for _, denominator in pairs)
...
def native_dram_data_bus_util(directory: Path) -> dict[str, float | int | str]:
    """Parse terminal native DRAM bwutil from retained raw logs; never windowize it."""
    ...
    if raw_gz.exists():
        ...
        return {"native_dram_data_bus_util": NA, "native_dram_n_cmd": NA}
    last_util: float | str = NA
    last_n_cmd: int | str = NA
    with source:
        for line in source:
            util = re.search(r"\bbwutil\s*=\s*([0-9.]+)", line)
            command = re.search(r"\bn_cmd=(\d+)", line)
            if util:
                last_util = float(util.group(1))
            if command and "bw_util=" in line:
                last_n_cmd = int(command.group(1))
    return {"native_dram_data_bus_util": last_util, "native_dram_n_cmd": last_n_cmd}
...
def load_contract(cell: Cell) -> dict[str, Any]:
    ...
    if contract.get("schema") != "EP_L2_CALIBRATION_CONTRACT_V1" or contract.get("cell") != cell.name:
        raise ValueError(f"{cell.name}: invalid cell contract {cell.contract_path}")
    if not isinstance(contract.get("effective_config"), dict) or not isinstance(contract.get("allowed_config_fields"), list):
...
def artifact_run(cell: Cell, directory: Path) -> dict[str, Any]:
    ...
    cardinality = {
        ...
        "cardinality_status": "PASS_FULL_WINDOWS_ONLY" if (len(l2_windows) == full_l2 * configured_l2 and
            ...
            dram_stream_status == "PASS_FULL_WINDOWS_ONLY") else "FAIL_TOPOLOGY_OR_STREAM_MISMATCH",
    ...
    return {"cell": cell.name, "workload": status["workload"], "variant": status["variant"],
            ...
            **native_dram_data_bus_util(directory), **cardinality, **temporal, **metrics}
...
def make_deltas(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in records:
        ...
        for metric in ("descriptor_need", "descriptor_block", "descriptor_occ_avg", "line_mshr_need", "line_mshr_block",
                       ...
                       "dram_read_bytes", "dram_write_bytes", "lower_admission_byte_rate_norm", "native_dram_data_bus_util",
...
def main() -> None:
    ...
    cardinality = [{key: row[key] for key in ("cell", "workload", "variant", "source_dir", "configured_l2_slices",
                   ...
                   "expected_dram_window_rows", "actual_dram_window_rows", "l2_stream_key_status", "dram_stream_key_status", "cardinality_status", "aggregation_reason")}
                   for row in records]
    temporal_keys = [
    ...
    write_csv(args.out / "NATIVE_DRAM_BANDWIDTH.csv", [{key: row[key] for key in ("cell", "workload", "variant", "native_dram_data_bus_util", "native_dram_n_cmd", "source_dir")} for row in records])
    write_csv(args.out / "CALIBRATION_MATRIX.csv", records)
    write_csv(args.out / "CALIBRATION_DELTAS.csv", make_deltas(records))
    (args.out / "ANALYSIS_MANIFEST.json").write_text(json.dumps({"schema": "EP_L2_LANE_D_V2", "cells": [{"name": cell.name, "root": str(cell.root), "descriptor_capacity": cell.descriptor_capacity, "l1_class": cell.l1_class, "contract": str(cell.contract_path)} for cell in args.cell], "records": len(records), "workloads": sorted(workloads) if workloads else "all"}, indent=2, sort_keys=True) + "\n")
