...
# 18. E3结果：Q30受控routing lightweight diagnostic

## Question

在固定Qwen3-30B-A3B、S2/T2048 Prefill Layer24、M=2048、E=128、k=8、dtype/backend和总assignment=16,384的条件下，仅改变routing组织，是否会显著改变whole-MoE-body native execution？

## Evidence

执行：

`hrl/c16-e3-q30-routing-diagnostic-109-v1@378487015e513ed666c0929ca3f6c00392ff11c3`

review pack：

`docs/vm_tlb/review_packs/C16_E3_Q30_ROUTING_DIAGNOSTIC_109_V1/`

本轮只使用native lightweight timing与routing/expert-group结构，不使用NVBit/NCU/NSYS/full trace。
...
- N：natural routing
- U：128 experts严格均衡
- H：固定top-8 hot experts极端集中
- P：保持natural route并联合置换token/order

P逆置换输出与N bitwise equal。
...
N/U median差约0.526%，小于本轮N、U各自约1.5–1.8%的run-to-run CV量级。

N router median约0.066015 ms。

Active experts / semantic active expert-MLP weight capacity：

- N：93 experts / 877,658,112 B
- U：128 experts / 1,207,959,552 B
- P：93 experts / 877,658,112 B
- H：8 experts / 75,497,472 B

注意：`SEMANTIC_ACTIVE_WEIGHT_CAPACITY`是参数容量，不是观测memory traffic。

## Interpretation

### N vs U

在这个固定Q30 shape/backend和本轮lightweight metrics下：

> balanced U是natural N的充分lightweight proxy。

尽管U激活更多expert、semantic active weight capacity更大，whole-MoE-body median没有出现超出本轮运行离散度的差异。

因此当前没有证据支持为N/U启动deep memory capture。

### P

P与N bitwise等价且timing相近。

本轮没有建立独立ordering effect。

### H

H约6.47 ms，明显快于N/U/P，但H同时改变：

- active expert count
- 每expert batch size
- non-empty expert-call数
- semantic active-weight capacity

因此H只证明极端hot-set route会改变整体执行行为。

它不能被解释为natural routing的cache/TLB瓶颈，也不能区分收益来自更大expert batch、更少expert calls、较小active weight capacity或其它backend效应。

## Reconciliation note：93 vs historical 92 active experts

早期accepted `S2_ROUTER_EXPERT_SUMMARY.tsv`曾记录Layer24 Prefill `unique_experts=92`。

本轮E3从同一accepted state重新得到93个active experts。

同时本轮：
- upstream state manifest闭合；
- natural router logits SHA为
  `c0a051b9bb1932a908a64eed039369bfc3cb22b0b9ad7bd94ec53979b5a80d28`，
  与accepted exact-layer replay的prefill router SHA一致；
- E3显式重构的N-body output与实际MLP output bitwise equal。

因此93是当前E3 harness对实际MLP执行语义的直接闭合结果；历史92与当前93之间存在一个**旧routing-census口径/实现差异待解释项**。

该差异不改变本轮N/U是在同一E3 harness、同一state下受控比较的事实，但后续若引用“natural active expert count”，应优先注明本轮93，并保留历史92差异，而不是静默覆盖。

## Superseded

此前假设：

> natural routing相对balanced proxy可能因为active-expert working-set差异而表现出显著整体执行差异。

在当前固定Q30 shape/backend和lightweight指标下，这一假设**未获支持**。

不代表其它shape/model一定无差异，但当前没有理由为本点继续深采。

## Next question

本MoE routing支线暂不自动进入deep trace或Cache/TLB机制。

后续主线应回到其它高信息增益、尚未回答的C16最小实验问题，而不是为了获得正结果继续扩大routing实验。
...
- N/U差异低于观测离散度；
- P未建立ordering effect；
- H仅提供极端synthetic sensitivity。

结论：

`STOP_LIGHTWEIGHT_E3_NO_DEEP_CAPTURE`
