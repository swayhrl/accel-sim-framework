...
| Static exception | The Qwen2.5-7B raw/AWQ pair has distinct verified checkpoint-file storage encodings; the AWQ catalog separates packed I32 payload from F16 scale/zero metadata. | A `TENSOR_STORAGE_CATALOG.tsv`, `WEIGHT_STORAGE_BREAKDOWN.tsv` | 2 deployments of one conceptual lineage; checkpoint-file weights | `STATIC_DERIVED`; not a dtype-causal or performance comparison, and never GPU PA. |
