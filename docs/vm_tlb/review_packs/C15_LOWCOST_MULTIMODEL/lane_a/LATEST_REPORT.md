...
Status: `C15_LOWCOST_FOUNDATION_PARTIAL_READY_FOR_FINAL_REVIEW`. `C15-1.6` and
...
All reported byte unions are checkpoint-file storage, never GPU VA/PA or runtime
allocation; `CHECKPOINT_FILE_STORAGE_SEMANTICS.md` records the column and alias
boundaries. MoE `intermediate_sizes` remain config scalars only and are excluded
from MoE clustering (`MOE_INTERMEDIATE_LIMITATIONS.md`).
...
Integration consumed B's hash-valid artifact manifest at
`57e2ef203befc96cfcefe00de2aaf8b0baab5d8b` and records B final handoff HEAD
`721e30f377dab36d826dc7ea9d47e11c5d85aa5c` separately; C's artifact/handoff is
