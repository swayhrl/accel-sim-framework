...
Open issues: an authorized visible GPU, compatible installed native backend, >=64 GiB disk reserve, >=32 GiB MemAvailable, verified local model revisions, a real profiler canary, and a read-only observer for lifetime V2. Raw-log index: none; C15 generated no raw logs.
